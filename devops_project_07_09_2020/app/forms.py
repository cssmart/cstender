...
class TASK_APPROVALS_FORM(forms.ModelForm):
    class Meta:
        model = TASK_APPROVALS
        fields = ['forwarded_note']

    def __init__(self, *args, **kwargs):
        super(TASK_APPROVALS_FORM, self).__init__(*args, **kwargs)
        self.fields['forwarded_note'].required = False
        self.fields['forwarded_note'].widget.attrs['style'] = 'width:100%; height:40px;'
...
class TASK_EDIT_FORM(forms.ModelForm):
    class Meta:
        model = TASK
        fields = ['task_name','description','assigned_by','assigned_to',
                  'status','start_date','end_date','expected_end_date','parent_task']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        super(TASK_EDIT_FORM, self).__init__(*args, **kwargs)
...
class TASK_FORM1(forms.ModelForm):
    class Meta:
        model = TASK
        fields = ['task_name','description','assigned_by','assigned_to','reassignable',
    ...
    def __init__(self, *args, **kwargs):
        ...
        # if self.fields['status'] is 'closed':
        #     self.fields['end_date'].required = True
        # else:
        #     self.fields['end_date'].widget = HiddenInput()
        if self.fields['status'] == 'closed':
            self.fields['end_date'].required = True
        self.fields['task_name'].required = True
        self.fields['description'].required = True
        # self.fields['parent_task'].required = False
        ...
        self.fields['end_date'].required = True
        # self.fields['end_date'].null = True
    ...
    # def clean(self):
    #     is_activity = self.cleaned_data.get('status', 'closed')
    #     if is_activity:
...
class PROJECT_RESOURCES_FORM(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['individual_resource_id'].required = True
        self.fields['resource_role'].required = True
