...
def Reinitialize_Approval_Workflow(request, pk):
    user_id = request.user.id
    user_form = UserForm.objects.filter(user_id=user_id).values('people_id')
    if user_form:
        p_id = user_form[0]['people_id']
        edit_task = TASK.objects.filter(id=pk).values('task_by', 'assigned_by',
                                                      'assigned_to', 'approval_flag')
        if p_id == f"{edit_task[0]['task_by']}" or (
                p_id == f"{edit_task[0]['assigned_by']}" or p_id == f"{edit_task[0]['assigned_to']}"):
            if edit_task[0]['approval_flag'] == 'Approved':
                message = 'This task is already approved. Take it easy!'
                return render(request, 'task_error_page.html', {'message': message})
            elif edit_task[0]['approval_flag'] == 'on_going':
                status_check = TASK_APPROVALS.objects.filter(task_id_id=pk,approval_status='awaiting').values('id')
                if status_check:
                    update_flag = TASK_APPROVALS.objects.filter(id=status_check[0]['id']).update(approval_status='reinitiated')
                    project_name = TASK.objects.filter(id=pk).values('project_id_id')
                    project_id = project_name[0]['project_id_id']
                    search_project = APPROVAL_HIERARCHY.objects.filter(project_id=project_id, approval_level=1). \
                        values('forwarded_by_id', 'forwarded_to_id')
                    if search_project:
                        create_task_approval = TASK_APPROVALS(approval_level='1', executed='Yes',
                                                              approval_status='awaiting',
                                                              forwarded_by_id=p_id,
                                                              forwarded_to_id=search_project[0]['forwarded_to_id'],
                                                              task_id_id=pk)
                        create_task_approval.save()
                    else:
                        message = "This Project does not exist in Approval Hierarchy"
                        return render(request, 'error_page.html', {'message': message})
            else:
                if edit_task[0]['approval_flag'] == 'Rejected':
                    project_name = TASK.objects.filter(id=pk).values('project_id_id')
                    project_id = project_name[0]['project_id_id']
                    search_project = APPROVAL_HIERARCHY.objects.filter(project_id=project_id,approval_level=1).\
                        values('forwarded_by_id','forwarded_to_id')
                    if search_project:
                        ...
                        message = "This Project does not exist in Approval Hierarchy"
                        return render(request, 'error_page.html', {'message': message})
            return redirect(f'/task_workflow_{pk}')
        else:
            message = 'You do not have sufficient privileges to Reinitialize the Approval Workflow '
            return render(request, 'error_page.html', {'message': message})
    else:
...
def task_summary_edit(request, pk):
    ...
    if search_approval_flag[0]['approval_flag'] == 'Approved' or search_approval_flag[0]['approval_flag'] == 'on_going':
        message = 'You do not have sufficient privileges to edit this task '
        return render(request, 'error_page.html', {'message': message})
    # else:
...
def home_my_task_list_view(request):
    ...
    if p_id:
        ...
        message = 'You do not have sufficient privileges to show this details '
        return render(request, 'error_page.html', {'message': message})
...
def task_workshow_in_task_summary(request, pk):
    task_approval = TASK_APPROVALS.objects.filter(task_id_id=pk).values('id', 'approval_level',
                                                                        'approval_status',
                                                                        'forwarded_by_id', 'forwarded_to_id',
                                                                        'forwarded_note', 'executed',
...
def home_approval_task(request):
    ...
    if p_id:
        ppl_id = p_id[0]['people_id']
        notification_table =NOTIFICATIONS.objects.filter(person_name_id=ppl_id).values('id','notification_text').\
            order_by('-id')
        task_search = TASK.objects.all().filter(assigned_to_id=ppl_id)
        ...
        for i in people:
            ...
            task_approve_details = TASK_APPROVALS.objects.filter(forwarded_to_id=id_,
                                        approval_status='awaiting').values('id', 'task_id','forwarded_by')
            t_id = []
            id_ = []
            forward_id = []
            p_id = []
        ...
        message = 'You do not have sufficient privileges to show the details '
        return render(request, 'error_page.html', {'message': message})


def notification_id_view(request,pk):
    search_in_notifications = NOTIFICATIONS.objects.filter(id=pk).values('notification_category','attribute_1')
    if search_in_notifications:
        attribute_val_1 = search_in_notifications[0]['attribute_1']
        category = search_in_notifications[0]['notification_category']
        if category == 'Project Resource Addition':
            return redirect(f'/all_task_list_{attribute_val_1}')
        elif category == 'Task Assignment Addition':
            task_table = TASK.objects.filter(id=attribute_val_1)
            return redirect(f'/task_detail_{attribute_val_1}')
        elif category == 'Task Final Approval':
            return redirect(f'/task_detail_{attribute_val_1}')
        elif category == 'DBA Task Assignment Addition':
            return redirect(f'/task_detail_{attribute_val_1}')
        else:
            if category == 'Task Final Rejection':
                return redirect(f'/task_detail_{attribute_val_1}')
    return HttpResponse('done')
...
def reject_approval_notification(request, pk):
    ...
    task_table = TASK.objects.filter(id=search_task[0]['task_id']).values('task_name', 'assigned_to_id')
    task_name = task_table[0]['task_name']
    assigned_to_ = task_table[0]['assigned_to_id']
    text_msg = f"Your task ({task_name}), has been rejected by the hierarchy."
    notification_create = NOTIFICATIONS(notification_category='Task Final Rejection',
                                        person_name_id=assigned_to_, notification_text=text_msg,
                                        attribute_1=search_task[0]['task_id'], created_by=user_login_fun(request),
...
def approval_notification_approved(request, pk):
    ...
    if approval_hierachy:
        ...
        task_name = task_table[0]['task_name']
        assigned_to_ = task_table[0]['assigned_to_id']
        text_msg = f"Your task ({task_name}), has been approved by the hierarchy."
        notification_create = NOTIFICATIONS(notification_category='Task Final Approval',
                                            person_name_id=assigned_to_, notification_text=text_msg,
                                            attribute_1=search_task[0]['task_id'], created_by=user_login_fun(request),
                                            last_updated_by=user_login_fun(request),
                                            creation_date=datetime_functionality(),
        ...
        try:
            if update_task:
                check_dba_processing = TASK.objects.filter(id=search_task[0]['task_id']).values('task_name',
                                    'description', 'task_by', 'assigned_by', 'assigned_to',
                                    'reassignable', 'status', 'parent_task', 'start_date', 'end_date',
                                    'expected_end_date','task_type', 'required_dba_processing', 'project_id_id')
                for i in check_dba_processing:
                    if i['required_dba_processing'] is True:
                        task_name_ = 'DBA Processing' + ' ( ' + i['task_name'] + ' )'
                        ...
                        query =f'''
                        SELECT resource_id
FROM public.app_project_resources where resource_role='dba' and project_id_id='{project}' ORDER BY RANDOM()
LIMIT 1;
                        '''
                        cursor.execute(query)
                        project_resource_view = cursor.fetchall()
                        if project_resource_view:
                            resource_id = str(project_resource_view).replace("[('",'').replace("',)]",'')
                            # resource_id = project_resource[0]['resource_id']
                            create_new_task = TASK(task_name=task_name_, description=description,
                            ...
                            table_task = create_new_task.id
                            id_ = ''.join(c for c in str(table_task) if c.isdigit())
                            task_data = TASK.objects.filter(id=id_).values('task_name','assigned_to_id')
                            assigned_to_id = task_data[0]['assigned_to_id']
                            task_name_data = task_data[0]['task_name']
                            text_msg = f"You have been assigned a new dba task, ({task_name_data})"
                            notification_create = NOTIFICATIONS(notification_category='DBA Task Assignment Addition',
                                                                person_name_id=assigned_to_id, notification_text=text_msg,
                                                                attribute_1=id_, created_by=user_login_fun(request),
                                                                last_updated_by=user_login_fun(request),
                                                                creation_date=datetime_functionality(),
                                                                last_update_date=datetime_functionality())
                            notification_create.save()
                        else:
                            message = 'Tasks with type movement are attached to parent tasks already approved'
                            return render(request, 'error_page.html', {'message': message})
        except:
            pass
    return redirect(f'/forwarded_note_{pk}')
...
def project_all_task_list(request, pk):
    ...
    task_list = TASK.objects.all().filter(project_id=pk).values('id', 'task_name',
                                                                ...
                                                                'task_complete', 'initial_expected_end_date').order_by(
        '-id')
    id_ = []
    task_name = []
    task_by_f_name = []
...
def complete_task_process(request, pk):
    ...
    FROM public.app_task where id = '{pk}'  and ((assigned_by_id='{p_id}' or assigned_to_id='{p_id}') or task_by_id ='{p_id}');
    '''
    ...
    if task_data:
        ...
        status_ = TASK.objects.filter(id=pk).values('status')
        status = status_[0]['status']
        if status != 'Closed':
            now = datetime.now()
            current_date = now.strftime("%Y-%m-%d")
            update_task = TASK.objects.filter(id=pk).update(status='closed',end_date=current_date)
            return redirect(f'/all_task_list_{project_id}')
        else:
            message='Task  has been closed'
            return render(request, 'error_page.html', {'message': message})
    else:
        message = "You do not have enough privileges to complete this task." + '\n' + 'you should be either an owner,assigner' \
...
#                 print(end_date,'sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
...
#                     assigned_by = None
#                 try:
#                     print(status,'sssssssssssssssssssssssssssssssssssssssssssssssss')
#                     if status =='closed':
#                         end_date=end_date
#                         print(end_date,'wwwwwwwwwwwwwwwwwwwwwwwwwwwwwww')
#                 except:
#                     end_date=None
#                     print(end_date,'2222222222222222222222222222222222222222222')
#                 # try:
#                 #     task_by = task_by.id
#                 # except:
#                 #     task_by = None
...
#                     else:
#                         if task_type == 'support' and required_dba_processing == True:
#                             messages.error(request, 'DBA processing requires task type change')
#                 elif status =='new' or status == 'in_progress':
#                     if start_date > expected_end_date:
#                         messages.error(request, 'Expected end date must be equal or greater than to start date')
#                     # elif end_date:
#                     #     end_date =None
#                     #     messages.error(request, 'End Date is not required if status is new or in progress.')
#
#                     else:
#                         if task_type == 'support' and required_dba_processing == True:
#                             messages.error(request, 'DBA processing requires task type change')
...
#                                            initial_expected_end_date=end_date,
...
#             else:
#                 print(form.errors, 'sssssssssssssssssssssssssssssssssssssssssssss')
...
#                                     elif end_date < expected_end_date:
#                                         messages.error(request,
#                                                        'Expected End date must be equal or less than to end date ')
#                                     elif task_type == 'support' and required_dba_processing == True:
#                                         messages.error(request, 'DBA processing requires task type change')
#                                     else:
#                                         pass
#                                 elif status == 'new' or status == 'in_progress':
#                                     if start_date > expected_end_date:
#                                         messages.error(request,
#                                                        'Expected date must be equal or greater than to start date')
#                                     elif end_date:
#                                         messages.error(request,
#                                                        'End Date is not required if status is new or in progress.')
#                                     elif task_type == 'support' and required_dba_processing == True:
#                                         messages.error(request, 'DBA processing requires task type change')
#                                     else:
#                                         pass
#                                 else:
#                                     task_table_save = TASK(project_id_id=project_id, task_name=task_name,
#                                                            task_by_id=people_id,
#                                                            assigned_by_id=assigned_by.id, assigned_to_id=assigned_to.id,
#                                                            start_date=start_date, end_date=end_date,
#                                                            description=description,
#                                                            status=status, reassignable=reassignable,
        ...
        if project_model[0][0] == 'self':
            ...
            if form.is_valid():
                ...
                if start_date > end_date or start_date > expected_end_date:
                    ...
                    request.session['id_'] = id_
                    if assigned_to:
                        # resource_id_ = PEOPLE.objects.filter(id=assigned_to).values('p_first_name', 'p_last_name')
                        # name = resource_id_[0]['p_first_name'] + ' ' + resource_id_[0]['p_last_name']
                        text_msg = f"You have been assigned a new task, ({task_name})"
                        notification_create = NOTIFICATIONS(notification_category='Task Assignment Addition',
                                                            person_name_id=assigned_to, notification_text=text_msg,
                                                            attribute_1=id_, created_by=user_login_fun(request),
                                                            last_updated_by=user_login_fun(request),
                                                            creation_date=datetime_functionality(),
                                                            last_update_date=datetime_functionality())
                        ...
                        if project_resrc[0]['resource_id'] == user_form[0]['people_id']:
                            ...
                            if form.is_valid():
                                ...
                                if start_date > end_date or start_date > expected_end_date:
                                    ...
                                    request.session['id_'] = id_
                                    if assigned_to:
                                        # resource_id_ = PEOPLE.objects.filter(id=assigned_to).values('p_first_name',
                                        #                                                             'p_last_name')
                                        # name = resource_id_[0]['p_first_name'] + ' ' + resource_id_[0]['p_last_name']
                                        text_msg = f"You have been assigned a new task, ({task_name})"
                                        notification_create = NOTIFICATIONS(
                                            notification_category='Task Assignment Addition',
                                            person_name_id=assigned_to, notification_text=text_msg,
                                            attribute_1=id_, created_by=user_login_fun(request),
                                            last_updated_by=user_login_fun(request),
                                            creation_date=datetime_functionality(),
...
def edit_task_form(request, **kwargs):
    ...
    if user_form:
        p_id = user_form[0]['people_id']
        edit_task = TASK.objects.filter(id=kwargs['pk']).values('approval_flag', 'task_by', 'assigned_by',
                                                                'assigned_to')
        if edit_task[0]['approval_flag'] == 'Approved' or edit_task[0]['approval_flag'] == 'on_going':
            ...
            if p_id == f"{edit_task[0]['task_by']}" or (
                ...
                if form.is_valid():
                    ...
                    form_data.save()
                    return redirect(f"/document_{project_idd}")
                context = {
                    "project_name": project_name,
                    "form": form,
                }
    ...
    if form.is_valid():
        ...
        if resource_role == 'owner':
            ...
            if not p_resource:
                ...
                table_task = project_resource_table.id
                if resource_id:
                    # resource_id_ = PEOPLE.objects.filter(id=resource_id).values('p_first_name', 'p_last_name')
                    # name = resource_id_[0]['p_first_name'] + ' ' + resource_id_[0]['p_last_name']
                    # id_ = ''.join(c for c in str(table_task) if c.isdigit())
                    project_name_ = PROJECT.objects.filter(id=pk).values('project_name')
                    proj_name = project_name_[0]['project_name']
                    text_msg = f"You have been added to a new project, ({proj_name})"
                    notification_create = NOTIFICATIONS(notification_category='Project Resource Addition',
                                                        person_name_id=resource_id,notification_text=text_msg,
                                                        attribute_1=pk,created_by=user_login_fun(request),
            ...
            if resource_id:
                ...
                notification_create.save()
            return redirect(f'/individual_resource_{pk}')
    resources_allocated = PROJECT_RESOURCES.objects.filter(project_id_id=pk).values('resource_id', 'resource_role')
    resource_role = []
    resource_id = []
    for i in resources_allocated:
        role = i['resource_role']
        resource_role.append(role)
        r_id = i['resource_id']
        p_id_ = PEOPLE.objects.filter(id=r_id).values('p_first_name', 'p_last_name')
        name = p_id_[0]['p_first_name'] + " " + p_id_[0]['p_last_name']
        resource_id.append(name)
    resources = zip(resource_role, resource_id)
            ...
            if people_table_check[0]['can_create_people'] is True:
                ...
                if self.form_1.is_valid():
                    ...
                    people_table = PEOPLE(p_first_name=p_first_name, p_last_name=p_last_name,
                                          ...
                                          can_create_application=can_create_application,
                ...
                if form.is_valid():
                    ...
                    if start_date > expected_end_date:
                        ...
                        project_table_save = PROJECT(project_name=project_name, application_id_id=application_id,
                                                     ...
                                                     initial_exp_completion_dt=expected_end_date,
