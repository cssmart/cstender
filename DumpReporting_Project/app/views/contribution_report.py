...
TABLE_NAME='contribution_report_2march2'
REPLACE_TABLE_NAME='contribution_report_28FEB'
...
def contribution_get_report_data(request):
    ...
    print('START TIME========',datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))
    ...
    print('TIME================', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))
    ...
    """
    print('TIME========list_data========', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))
    cursor_ora = conn_ora.cursor()
    print(cursor_ora, 'Connection establish===============================================')
    cursor_ora.arraysize=1000

    print(cursor_ora,'lkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
    cur = cursor_ora.execute(list_data)
    print('TIME====CUR EXECUTE========', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))
    # cursor_ora.arraysize=1000
    for i in cur:
        print(i,'ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
    # contribution_report = cur.fetchall()
        print('TIME========contribution_report========', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))

    print('TIME========contribution_report========', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))
    #
    # print(contribution_report,'data fetch==============================================================')
    # print('TIME====contribution_report_END========', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))
    #
    # df = pd.DataFrame(contribution_report)
    # csv_data = df.to_csv(f'{TABLE_NAME}.csv', sep='|', header=False, index=False)
    # print('TIME====csv_data========', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))
    #
    # print(csv_data,'csv created =======================================================================')
    ...
    try:
        ...
        if table_type == 'insert':
            ...
            try:
                with open(f'{TABLE_NAME}.csv', 'r') as f:
                    ...
                    cursor.copy_from(f, f'{TABLE_NAME}', sep='|')
                    print(cursor)
                    conn_db.commit()
        ...
        if table_type == 'create&insert':
            ...
            try:
                cursor.execute(table_create)
                print('TIME==TABLE CREATE',datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))

                print(cursor,' Table created======================================')
                conn_db.commit()
                print('table commit---------------------------------------------------')
                with open(f'{TABLE_NAME}.csv', 'r') as f:
                    # next(f)  # Skip the header row.
                    cursor.copy_from(f, f'{TABLE_NAME}', sep='|')
                    print('TIME====DATA INSERT========', datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))

                    print(cursor,'Insert Data-=====================================================================')

                    conn_db.commit()
                    print('TIME====DATA COMMIT========',datetime.strptime(datetime.now().strftime('%Y-%m-%d %H %M %S'), '%Y-%m-%d %H %M %S'))

                    print('Data Insert Succ==============================================')
