...
from app.forms import DumpForm, DumpDownloadForm
...
TABLE_NAME='avc33'
REPLACE_TABLE_NAME = 'Report_new112122w'


table_create = f""" CREATE TABLE {TABLE_NAME} (EXCHANGE_RATE VARCHAR(300),
...
def dump_report_data(request):
    ...
    V.CREDIT_MEMO_NUM,
    V.CREDIT_MEMO_DATE"""
    # data_1 = "SELECT * FROM gl_currencies"
    cur = cursor_ora.execute(data)
    list_data = []
    ...
    conn_db = psycopg2.connect(user="oracle_user", password="password", host="127.0.0.1", port="5433",
                               database="oracle_db")
    postgre_cursor = conn_db.cursor()
    try:
        if table_type == 'create&insert':
            try:
                sql_statement =table_create
                postgre_cursor.execute(sql_statement)
                conn_db.commit()
                ...
                sql_stat = f"""INSERT INTO {TABLE_NAME} (EXCHANGE_RATE,
                            ...
                            CREDIT_MEMO_DATE)
                            VALUES {res}"""
                postgre_cursor.execute(sql_stat)
                conn_db.commit()
                return JsonResponse({"Message": f" Table  {TABLE_NAME} Created and Data inserted in {TABLE_NAME} Successfully"})
            except Exception as e:
                return HttpResponse(e)

        elif table_type == 'create_table':
            try:
                create_statement =table_create
                postgre_cursor.execute(create_statement)
                conn_db.commit()
                print("Table created successfully in PostgreSQL ")
            ...
            print(table_type)
            try:
                sql_stat =f"""INSERT INTO {TABLE_NAME} (EXCHANGE_RATE,BUSINESS_UNIT,
                            BUSINESS_LINE,
                            EVENT_CLASS_CODE,
                            ENTITY_CODE,
                            DEL_ORG_ID,
            ...
            try:
                ...
                postgre_cursor.execute(drop_table)
                conn_db.commit()
                sql_statement=table_create
                print(sql_statement)
                postgre_cursor.execute(sql_statement)
                ...
                return JsonResponse({"Message": f"Data Replaced from {REPLACE_TABLE_NAME} &" f"Created and Inserted data in {TABLE_NAME} Successfully"})
            except Exception as e:
                return JsonResponse({"Error": e})
...
def download_dump_report(request):
    ...
    and ('{business_line}'='' OR business_line = '{business_line}')
    and ('{business_unit}'='' OR business_unit = '{business_unit}');
'''
    ...
    if report_type == 'excel':
        ...
        headings = ['EXCHANGE_RATE', 'BUSINESS_UNIT', 'BUSINESS_LINE', 'EVENT_CLASS_CODE', 'ENTITY_CODE', 'DEL_ORG_ID',
            ...
            'OA_NO','ORDER_TYPE','CUR','NET', 'GROSS','GROSS_WEIGHT','HEADER_ID','SP_INS', 'VOLUME','RAT_TERM_NAME', 'FOB',
            'GST_DATE','GST_TIME', 'INVENTORY_ITEM_ID', 'ITEM_CODE','DESCRIPTION','UNIT_CODE', 'LINE_NUMBER', 'QUANTITY',
            'UNIT_PRICE' , 'CONVERSION_RATE', 'WAYBILL', 'TRX_NUMBER', 'REVENUE_AMOUNT', 'REV_UNIT', 'REV_ACCOUNT',
            'REV_ACCT_DESC','SHIPPING_ORG', 'CUSTOMER_NUMBER', 'SHIP_CUSTOMER', 'SHIP_CUST_GSTN', 'SHIP_CUST_STATE',
            'CUSTOMER_CLASS_CODE', 'CUSTOMER_CATEGORY_CODE','DOM_IBD', 'ZONE', 'REGION', 'BRANCH', 'COUNTRY','SALESREP_ID',
            'COMM','TRX_CLASS', 'TAXABLE_VALUE', 'TAX_INVOICE_NUM', 'RETURN_REASON_CODE', 'RETURN_ORDER_NUM', 'RETURN_INVOICE_NUM',
            'TAX_INVOICE_DATE', 'TRX_ID', 'TRX_LINE_ID', 'ORG_ID', 'SUPPLIER_GST_NUMBER', 'SUPPLIER_PAN_NUM', 'CUSTOMER_GST_NUM',
            'CUSTOMER_PAN_NUM','CGST', 'CGST_RATE', 'SGST', 'SGST_RATE','IGST', 'IGST_RATE', 'CURRENCY_CONVERSION_RATE',
            'REGIME_CODE','GST_EVENT_CLASS_CODE', 'GST_ENTITY_CODE', 'DET_FACTOR_ID','SUPP_STATE', 'BILL_STATE','FREIGHT',
            'INSURANCE','PACKING','TCS','SALES_PERSON','ORDER_CLASS','MIS_CAT1', 'MIS_CAT2',
            'HSN_SAC','ADDRESSEE','TAX_CATEGORY_NAME','CREDIT_MEMO_NUM','CREDIT_MEMO_DATE']
        col = 0
