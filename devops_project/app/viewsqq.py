...
from .forms import PEOPLE_FORM,TEAM_FORM1,TEAM_FORM2,PROJET_FORM,PROJECT_RESOURCES_FORM, \
    ...
    DOCUMENT_FORM,TASK_FORM1,SignUpForm,EDIT_PROJET_FORM
...
def approval_notification_approved(request, pk):
    ...
    if approval_hierachy:
        ...
        if update_task:
            check_dba_processing = TASK.objects.filter(id=search_task[0]['task_id']).values('task_name','description', 'task_by','assigned_by','assigned_to','reassignable',
                  'status','parent_task','start_date','end_date','expected_end_date','task_type','required_dba_processing','project_id_id')
            for i in check_dba_processing:
                if i['required_dba_processing'] is True:
                    ...
                    status=i['status']
                    today = date.today()
                    prjt_resrc = PROJECT_RESOURCES.objects.filter(project_id_id=project,resource_role='dba').values('resource_id').order_by('-id')[:1]
                    if prjt_resrc:
                        resource_id = prjt_resrc[0]['resource_id']
                        create_new_task = TASK(task_name=task_name,description=description,
                        task_by_id=task_by,assigned_by_id=assigned_by,assigned_to_id=resource_id,reassignable=True,status=status,
                        start_date =today,task_type='Movement',parent_task_id=parent_task,project_id_id=project,
                       ...
                       last_update_date=datetime_functionality())
                        ...
                        message='Please select the role DBA in project resource.'
                        return render(request, 'error_page.html', {'message': message})
    return redirect('/approvals_list')
...
def parent_task_form_view(request,pk):
    ...
    for i in task_details_:
        ...
        task_by_name = task_by[0]['p_first_name']+' '+task_by[0]['p_last_name']
        assign_by_name = assign_by[0]['p_first_name']+' '+assign_by[0]['p_last_name']
        assign_to_name = assign_to[0]['p_first_name']+' '+assign_to[0]['p_last_name']
        context = {
            "task": task,
            "document": document,
            "assign_to": assign_to,
...
def task_details_view(request,pk):
    ...
    cursor = connection.cursor()
    task_details = f'''SELECT task_by_id, assigned_by_id,assigned_to_id,parent_task_id
        FROM public.app_task where id='{pk}';
    '''
    cursor.execute(task_details)
    task_details_ = cursor.fetchall()
    for i in task_details_:
        ...
        assign_by_name = assign_by[0]['p_first_name']+' '+assign_by[0]['p_last_name']
        assign_to_name = assign_to[0]['p_first_name']+' '+assign_to[0]['p_last_name']
        ur = f'http://127.0.0.1:8000/task_detail_{parent_id}'
        context = {
            "parent_id":parent_id,
            "ur":ur,
            "task": task,
...
def project_all_task_list(request,pk):
    ...
    'expected_end_date','task_type','task_complete').order_by('-id')
    id_=[]
    task_name =[]
    task_by_f_name=[]
    task_by_l_name=[]
    ...
    task_list_ = zip(id_, task_name,status,start_date,end_date,expected_end_date,task_type,reassignable,
                     ...
                     assign_to_l_name,task_complete)
    ...
    if id_ and project_id:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                create_doc = DOCUMENT(document_name=document_name, document_location=document_location,document_type=document_type,
...
class PEOPLE_FORM_VIEW(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form_1.is_valid():
            ...
            people_table = PEOPLE(p_first_name=p_first_name,p_last_name=p_last_name,can_create_people=can_create_people,
                ...
                if form.is_valid():
                    ...
                    if start_date > expected_end_date:
                        ...
                        project_table_save = PROJECT(project_name=project_name,application_id_id=application_id,
                                                     ...
                                                     self_task_reasignment=self_task_reasignment,project_business_owner=project_business_owner,
                                                     created_by=user_login_fun(request), last_updated_by=user_login_fun(request),
                                                     creation_date=datetime_functionality(),project_objective=project_objective,
                                                     last_update_date=datetime_functionality()
                                                     )
...
def edit_project_form(request, **kwargs):
    ...
    for i in queryset:
        data = queryset.update(project_type=i.project_type,expected_end_date=i.expected_end_date,
                              project_description=i.project_description,status=i.status,task_creation=i.task_creation,
                               self_task_reasignment=i.self_task_reasignment,project_business_owner=i.project_business_owner,
