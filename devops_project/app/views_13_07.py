...
class FileFieldView(FormView):
    form_class = TASK_DocumentForm
    template_name = 'upload.html'  # Replace with your template.
    success_url = '...'  # Replace with your URL or reverse().

    def post(self, request, *args, **kwargs):
        print(args,kwargs,'sssssssssssssssssssssssssssssss')
        form_class = self.get_form_class()
        form = self.get_form(form_class)
        files = request.FILES.getlist('document_upload')
        print(files,'ffffffffffffffffffffffffffffffffffffffffff')
        if form.is_valid():
            d  = form.save(commit=False)
            print(d.read(),'ooooooooooooooooooooooooooooooooooooo')
            for f in files:
                print(f,'pppppppppppppppppppppppppp')
                d.save()
...
def task_form_view(request):
    ...
    if form.is_valid():
        form_1 = form.save(commit=False)
        print(form_1,'sssssssssssssssssssss')
        print(form_1.id,'ssssssssssssssssssssssssssssssssssssss')
        ...
        print(parent_task,'kkkkkkkkkkkkkkkkkkkkkkkkkkkk')
        task_table_save = TASK(project_id=project_id.id,task_name=task_name, task_by_id=task_by.id,
                               ...
                               status=status, reassignable=reassignable, parent_task=parent_task.id,
        ...
        print(task_table_save,'lkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
        table_task = task_table_save
        id_ = ''.join(c for c in str(table_task) if c.isdigit())
        request.session['id_'] = id_
        print(request.session['id_'],'mmmmmmmmmmmmssssssssssssssssssssssssssssss')
        return redirect("/progress-bar-upload/")
...
class ProgressBarUploadView(View):
    def get(self, request,):
        ...
        try:
            photos_list = TASK.objects.all().filter(id='111')
            print(photos_list,'11111111111111111111111111111111111111111111111')
        except:
            photos_list = ['']
            print(photos_list,'=================================')
    ...
    def post(self, request):
        path = join(settings.MEDIA_ROOT + '\\Document_Upload_Folder\\')
        # print(path, 'dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
        # time.sleep(1)
        # form = TASK_DocumentForm(request.POST, request.FILES)
        # if form.is_valid():
            # document_upload = request.FILES['document_upload']
            # print(document_upload,'8888888888888888888888888888888888888888888888888888')
            # file_loc = path + str(document_upload)
            # content = document_upload.read()
            # f = open(file_loc, "w")
            # f.write(str(content))
            # f.close()
            # for f in request.FILES.getlist('document_upload'):
            #     TASK.objects.filter(id='111').update(document_upload=f)
            #     # TASK.objects.create(document_upload=f)
            # return HttpResponse('OK')

        time.sleep(1)  # You don't need this line. This is just to delay the process so you can see the progress bar testing locally.
        form = TASK_DocumentForm(self.request.POST, self.request.FILES)
        print(form,'fffffffffffffffffffffffffffffffffffffffffffff')
        id_ = request.session.get('id_')
        print(id_,'id-----------------------------------------------------------------')
        if form.is_valid():
            photo = form.save(commit=False)

            print(photo,'wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww')
            document_upload = form.cleaned_data['document_upload']
            print(document_upload,'llllllllllllllllllllllllllllllllllllllllllllll')
            ...
            f.close()
            # for f in request.FILES.getlist('document_upload'):
            #     TASK.objects.filter(id=id_).update(document_upload=f)
            #     # TASK.objects.create(document_upload=f)
            cursor = connection.cursor()
            doc_ = f'''SELECT id , document_location
	                FROM public.app_task where id='{id_}';
                    '''
            cursor.execute(doc_)
            row = cursor.fetchall()
            print(row,'dddddddddddddddddddddddddddddd3333333333333333333333333')
            for i in row:
                doc_location = [f'{i[1]}']
                new_doc = [f'{document_upload}']
                doc_location.extend(new_doc)
                update_location = str(doc_location).replace("'", '').replace('[', '').replace(']', '')
                print(update_location,'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')

                task_ = f'''UPDATE public.app_task
                SET   document_upload='{document_upload}',document_location='{update_location}'
                WHERE id = '{id_}';
                                      '''
                cursor.execute(task_)
                print('hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh',cursor)
            # # photo.save()
            return redirect('.')
        else:
            data = {'is_valid': False}
