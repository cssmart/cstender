...
class asset_asset(models.Model):
    ...
    criticality = fields.Selection(CRITICALITY_SELECTION, 'Criticality', track_visibility='onchange')
    property_stock_asset = fields.Many2one(
        'stock.location', "Asset Location",
        company_dependent=True, domain=[('usage', 'like', 'asset')], track_visibility='onchange',
    ...
    admin_asset_id = fields.Many2one('admin.assets', string='Admin Id')
    # user_id = fields.Many2one('res.users',store=True, related='finance_asset_id.serial', 'Assigned to', track_visibility='onchange')
    active = fields.Boolean('Active', default=True)

    model = fields.Char('Model', size=64)
    serial = fields.Char('Serial no.', size=64)
    finances_asset_id = fields.Many2one('account.asset.asset', required=True, string='Finance Id')
    asset_number = fields.Char('Asset Number', size=64)
    # model = fields.Char(related='finances_asset_id.model')
