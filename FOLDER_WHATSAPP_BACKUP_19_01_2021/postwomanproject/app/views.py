...
from .models import SESSION_WINDOW, INTERACTION_OUT, PALETTE_TABLE, WHATSAPP_MAIL, \
    General_configuration_chatbot, PALETTE_STRUCTURE, INTERACTION_IN_TABLE, abc,Mobile_No
...
Your order number {order_no} dated {order_date} for amount INR {order_amount} has been booked into the system.
...
def whatsapp_chatbot(request):
    ...
    msg_id = data['uid']
    opening_keywords = ['#']
    mobile_no_check = Mobile_No.objects.filter(mobile_number = from_chat)
    if mobile_no_check:
        ...
        return JsonResponse({'Message': 'Number does not exist in table'})
                ...
                for exce_query in function_table_call_parameters:
                    if exce_query[1] == 'oracle':
                        if text_message:
                            ...
                            TEXT_MSG = function_receive_data(function_query_execute)
                            file = open(f"MesageResponses/{from_chat}.txt", "w")
                            file.write(str(TEXT_MSG))
                            file.close()
