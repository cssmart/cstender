...
def task_workshow_in_task_summary(request, pk):
    ...
    if task_approval:
        ...
        approval_data = zip(approval_id, approval_level, executed, approval_status, forwarded_by, forwarded_to)
        task = TASK.objects.filter(id=pk).values('task_name', 'project_id_id')
        task_name = task[0]['task_name']
        project_id = task[0]['project_id_id']
        approval_hierarchy = APPROVAL_HIERARCHY.objects.filter(project_id_id=project_id).values('id', 'approval_level',
                                                                                                'type',
                                                                                                'forwarded_by_id',
                                                                                                'forwarded_to_id',
