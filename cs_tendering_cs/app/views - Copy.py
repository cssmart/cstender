...
def add_module_list(request, **kwargs):
    new_item = get_object_or_404(models.BoardDetails,  pk=kwargs['pk'])
    print(new_item,'kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
    queryset = models.BoardDetails.objects.select_related().filter(pk=kwargs['pk'])
    print(queryset,'pppppppppppppppppppppppppppppppppppppppppppppppppppppppppp')
    ...
    if form.is_valid():
        form_data = form.save(commit=False)
        form_data.board_detail = new_item
        # for i in queryset:
    ...
    return render(request, 'app/app/add_component122.html',context,  {
...
def add_component_list(request, **kwargs):
    ...
    # new_cd = models.ComponentDetails()
    # new_cd.board_detail = new_item
    # print(new_cd.board_detail, '111111111111111111')
    # # new_cd.save()
    # # print(new_cd.save(),'sssssssssssssssssssss')
    # for i in queryset:
    #     print(i.total_quantity,'11111111111333333333322222222222222')
    #     new_cd = models.ComponentDetails()
    #     new_cd.board_detail =str(i.board_detail_id)
    #     print(new_cd.board_detail,'111111111111111111')
    #     new_cd.save()
    form = forms.ComponentDetailForm(request.POST or None, instance=new_item)
    ...
    if form.is_valid():
        ...
        component_id = form_data.cleaned_data['component_id']
        description = form_data.cleaned_data['description']
        quantity = form_data.cleaned_data['quantity']
        print(quantity, 'quantityffffffffffffffffffff')
        p = models.ComponentDetails(board_detail=new_item, component_id=component_id, quantity=quantity,
                                    description=description, tender_id=tender_id_data)
        print(p, 'ddddddddddddddddddddddddddd')
        p.save()
        # form.save()
    ...
    component_data = models.ComponentDetails.objects.filter(tender_id=tender_id_data)
    return render(request, 'app/app/add_component.html', context, {'component_data': component_data})
